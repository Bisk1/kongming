...
from chinesetool.models import WordZH, WordPL, SentenceZH, WordTranslation, Abonament
...
def result(request, status):
    if int(status) == 1: # conversion to int required
...
def ajax(request):
   if request.POST.has_key('client_response'):
        ...
        if lesson.number > 0 and lesson.number < 20:
...
class LessonController():
    ...
    def to_object(object_dict):
        if object_dict.has_key("number"):
            number = object_dict["number"]
        else:
            number = 0
        if object_dict.has_key("fails"):
            fails = object_dict["fails"]
        else:
            fails = 0
        if object_dict.has_key("current_word"):
            current_word = WordZH.objects.filter(pk= object_dict["current_word"])[0]
        else:
            current_word = None
        words = list()
        if object_dict.has_key("words"):
            for i in range(20):
